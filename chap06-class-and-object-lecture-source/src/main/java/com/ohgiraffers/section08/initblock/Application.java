...
public class Application {
    public static void main(String[] args) {
        /*초기화블럭의 동작 순서를 이해하고 이를 활용하여 인스턴스 생성
        * 초기화 블럭
        * 복잡한 초기화를 수행할 수 있는 블럭을 제공하며,
        * 인스턴스 초기화 블럭과 정적 초기화 블럭으로 구분이 된다.
        * 1. 인스턴스 초기화 블럭
        * : 인슽ㄴ스가 생성되는 시점에 생성자 호출 이전에 먼저 실행이 된다
        * 인스턴스를 호출하는 시점마다 호출이 된다
        * 인스턴스 변수를 초기화 하며 정적 필드에는 실행시점마다 값을 덮어버린다
        * (표현식)
        * {
        *   초기화내용
        * }
      ...
      //  System.out.println(Product.getInfo());
        //2. 명시적 ㅊ로기화로 필드 초기화 확인
        //명시적으로 초기값을 작성하면 -> 가본값 -> 명시적 초기값으로 덮어쓴다
        System.out.println(product.getInfo());
        ...
        * 클래스 변수 : 기본값 -> 명시적 초기값 - > 정적초기화블럭 -> 인스턴스 초기화블럭-> 생성자
        * */
