...
public class FinalFieldTest {
    /*final 키워드에 대해 조금 더 자세히 이해할 수 있다.
    ...
    * 4. 클래스 변수 : 프로그램 start 이후 값 변경 불가
    * 5. non-static 메소드 : 메소드 제작성(overriding) 불가
    ...
    */
    //인스턴스가 생성 될 떄 int 자료형의 기본값이 0이 필드에 들어가는데
    //초기화 이후 값을 변경할수없기떄문에 바로 초기화를 시켜주어야 한다.
    ...
    //private  static final int STATIC_NUM; // 0 이라는 기본값이기 떄문에 오류 발생
    private static  final double PI = 3.14;
/*        STATIC 이붙은 키워드는 생성자를 이용한 초기화가 불가능하다.
*           생성자는 인스턴스가 생성되는 시점(NEW를 만나는 시점) 에 호출이 되기 떄문에
*           그 전에는 초기화가 일어나지 못 한다
*           하지만 static 키워드는 프로그램이 run 하는 동시에 할당되기 떄문에
*           초기화가 되지 않은 상태로 선언 된 것과 동일하여
*           기본값으로 초기화 된 후 값을 변경하지 못 하기 떄문에 에러가 발생한다 */
